...
class FireSeparationRule(Rule):
    rule_id = "fire-separation-rating"
    ...
    def evaluate(
        ...
        for item in items:
            ...
            if item_type_value != FireProtectionItemType.FIRE_SEPARATION.value:
                continue
            ...
            if required_minutes is None or provided_minutes is None:
                passed = False
                message = (
                    f"Fire separation {item_id} ({location}) has invalid or missing "
                    f"ratings (required: {rating_required!r}, provided: {rating_provided!r})."
                )
            elif provided_minutes >= required_minutes:
                passed = True
                message = (
                    f"Fire separation {item_id} ({location}) provided rating "
...
fire_separation_rule = register_rule(FireSeparationRule())
